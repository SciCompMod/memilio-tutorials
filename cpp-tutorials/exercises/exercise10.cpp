...
mio::osecir::Model<ScalarType> create_model()
{
    ...
    // EXERCISE: Set up the ContactMatrix for each location with the correct baseline and (where applicable)
    // minimum values. The baselines should sum to 10 contacts/day, matching Tutorial 3.
    //   Home:   baseline = 4, minimum = 1  (irreducible household contacts)
    //   School: baseline = 3
    //   Work:   baseline = 2
    //   Other:  baseline = 1
    // Hint: use mio::ContactMatrix<ScalarType>(Eigen::MatrixX<ScalarType>::Constant(1, 1, <value>))
    //       or the two-argument form for baseline + minimum.
    // contacts[(size_t)Location::Home]   = ???
    // contacts[(size_t)Location::School] = ???
    // contacts[(size_t)Location::Work]   = ???
    // contacts[(size_t)Location::Other]  = ???
...
int main()
{
    ...
    // Hint: cm[(size_t)Location::School].add_damping(<coeff>, mio::SimulationTime<ScalarType>(t_npi_start));
    // ???
