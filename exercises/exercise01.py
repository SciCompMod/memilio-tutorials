...
def _(mo):
    mo.md(r"""
    ### Exercise
    Please set the average time individuals spend in the exposed state (`TimeExposed`) to 4 days.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### Exercise
    Please set 2% of the population initially infected. The initially infected should be distributed to the compartpartments as follows: 0.5% is initially exposed (state `Exposed`), 0.5% is non-symptomatically infected (state `InfectedNosymptoms`) and 1% is symptomatically infected (state `InfectedSymptoms`).
    """)
    return
