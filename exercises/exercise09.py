...
def _(mio):
    def set_covid_parameters(model):
        ...
        for ag, val in enumerate([0.40, 0.35, 0.30, 0.30, 0.25, 0.20]):
            model.parameters.RecoveredPerInfectedNoSymptoms[mio.AgeGroup(ag)] = val
        ...
        for ag, val in enumerate([0.10, 0.10, 0.10, 0.08, 0.08, 0.05]):
            model.parameters.TransmissionProbabilityOnContact[mio.AgeGroup(ag)] = val

        # Relative transmission for asymptomatic and symptomatic cases
        for ag in range(6):
            model.parameters.RelativeTransmissionNoSymptoms[mio.AgeGroup(ag)] = 1.
            model.parameters.RiskOfInfectionFromSymptomatic[mio.AgeGroup(ag)] = 1.
...
def _(mio, os):
    def set_contact_matrices(model, damping_start, damping_value):
        ...
        # Use: contact_matrices[0].add_damping(mio.Damping(coeffs=..., t=...))
...
def _(mio, osecir, t0):
    def set_populations(graph, model):
        ...
        susceptible_pops = [
            ...
            [820000,  1730000, 4140000, 6040000, 3770000, 1530000],  # Region 3 (seeded)
        ...
        for node_id, pops in enumerate(susceptible_pops, start=1):
            ...
            # TODO: Seed the outbreak in Region 3 only (node_id == 3).
            # Subtract 100 from age group 2's Susceptible count and
            # set age group 2's Exposed count to 100.
    ...
    def set_mobility(graph):
        ...
        for i in range(graph.num_nodes):
            for j in range(i + 1, graph.num_nodes):
                total_i = graph.get_node(i).property.model.populations.get_total()
                total_j = graph.get_node(j).property.model.populations.get_total()
...
def _(
    ...
    # These indices select the Critical and Dead compartments for each age group
    # from the full state vector. Each age group has 10 compartments; Critical is
    # at position 10*i-3 and Dead at 10*i-1 (1-indexed).
    ...
    def run_simulation(damping_start, damping_values):
        ...
        # TODO: Apply per-region contact dampings by looping over graph.num_nodes.
        # For each node_idx, retrieve it with graph.get_node(node_idx), then call
        # set_contact_matrices(...) on node.property.model, passing the
        # corresponding entries from damping_start and damping_values.

        # Run the ODE-based simulation
        sim = osecir.MobilitySimulation(graph, t0, dt=0.5)
        sim.advance(tmax)

        # TODO: Extract results for each node into a dict.
        # Loop over sim.graph.num_nodes. For each node_idx:
        #   1. Retrieve the node with sim.graph.get_node(node_idx)
        #   2. Call osecir.interpolate_simulation_result(node.property.result)
        #      and convert to a numpy array
        #   3. Select only the columns in compartment_indices and round to integers
        #   4. Store under results[f'region{node_idx}']
        # Return the completed results dict.
        results = {}
...
def _():
    ...
    def prior():
        ...
        """
        # TODO: Return a dict with two keys: 'damping_start' and 'damping_values'.
        # Each should be a numpy array of shape (5,) sampled from a uniform
        # distribution using the corresponding entry in bounds.
        pass

    # Quick sanity check — you should see two arrays of length 5
    sample = prior()
    print("Prior sample:")
...
def _(mo):
    ...
    The adapter here needs to:
    1. Convert all arrays to float32 (from float64).
    2. **Constrain** the inferred parameters to their prior bounds — this ensures the normalising flow maps to the correct range.
    3. **Concatenate** the two parameter vectors (`damping_values`, `damping_start`) into a single `inference_variables` tensor.
    4. **Concatenate** all five region outputs into a single `summary_variables` tensor for the summary network.
    """)
    return
...
def _(mo):
    ...
    We train online for 15 epochs with a batch size of 64. The trained model is saved so it can be reloaded without retraining.
    """)
    return
...
def _(load_data, np, run_simulation, workflow):
    ...
    print(f"  damping_start  shape: {posterior_samples['damping_start'].shape}")
    print(f"  damping_values shape: {posterior_samples['damping_values'].shape}")
    print()
...
def _(np, plt):
    def plot_aggregated_over_regions(
        ...
        if data.ndim != 3:
            raise ValueError("data must have shape (samples, time_points, regions)")
        ...
        ax.fill_between(x, qs_50[0], qs_50[1], alpha=0.6, linewidth=0, color=color, label="50% CI")
        ax.fill_between(x, qs_90[0], qs_90[1], alpha=0.35, linewidth=0, color=color, label="90% CI")
        ax.fill_between(x, qs_95[0], qs_95[1], alpha=0.15, linewidth=0, color=color, label="95% CI")

        if true_data is not None:
            true_vals = region_agg(true_data, axis=-1)
            ax.scatter(x, true_vals, color="black", label="Observed data", marker="x", zorder=3, s=40)
    ...
    def plot_region_fit(data, region, true_data=None, ax=None, label=None, color="steelblue"):
        ...
        ax.fill_between(x, qs_80[0], qs_80[1], alpha=0.5, color=color, label="80% CI")
        ax.fill_between(x, qs_95[0], qs_95[1], alpha=0.2, color=color, label="95% CI")

        if true_data is not None:
            ax.plot(x, true_data[:, region], lw=1.5, color="black", label="Observed")
...
def _(observed_data, plot_region_fit, plt, simulations):
    fig_grid_crit, ax_grid_crit = plt.subplots(5, 6, figsize=(20, 16), sharex=True)
    ...
    for _region in range(5):
        for _ag in range(6):
            ...
            if _ag == 0:
                ax_grid_crit[_region, _ag].set_ylabel(f"Region {_region + 1}", fontsize=9)
...
@app.cell
def _(observed_data, plot_region_fit, plt, simulations):
    fig_grid_dead, ax_grid_dead = plt.subplots(5, 6, figsize=(20, 16), sharex=True)
    ...
    for _region in range(5):
        for _ag in range(6):
            ...
            if _ag == 0:
                ax_grid_dead[_region, _ag].set_ylabel(f"Region {_region + 1}", fontsize=9)
