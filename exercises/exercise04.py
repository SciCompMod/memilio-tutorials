...
def _():
    ...
    return contact_frequency, t0, tmax
...
@app.cell
def _(osecir):
    # TODO: Create a model with one age group
    model = osecir.Model()
    return
...
app._unparsable_cell(
    ...
    def set_parameters(model, parameters):
        # TODO: We need to set the parameters for our one age group. Please add the missing parts:
    
        model.parameters.TimeExposed[] = parameters["TimeExposed"]
        model.parameters.TimeInfectedNoSymptoms[] = parameters["TimeInfectedNoSymptoms"]
        model.parameters.TimeInfectedSymptoms[] = parameters["TimeInfectedSymptoms"]
        model.parameters.TimeInfectedSevere[] = parameters["TimeInfectedSevere"]
        model.parameters.TimeInfectedCritical[] = parameters["TimeInfectedCritical"]

        model.parameters.RelativeTransmissionNoSymptoms[] = parameters["RelativeTransmissionNoSymptoms"]
        model.parameters.TransmissionProbabilityOnContact[] = parameters["TransmissionProbabilityOnContact"]
        model.parameters.RecoveredPerInfectedNoSymptoms[] = parameters["RecoveredPerInfectedNoSymptoms"]
        model.parameters.RiskOfInfectionFromSymptomatic[] = parameters["RiskOfInfectionFromSymptomatic"]
        model.parameters.SeverePerInfectedSymptoms[] = parameters["SeverePerInfectedSymptoms"]
        model.parameters.CriticalPerSevere[] = parameters["CriticalPerSevere"]
        model.parameters.DeathsPerCritical[] = parameters["DeathsPerCritical"]
    """,
    name="_"
)
...
app._unparsable_cell(
    ...
    def set_population(model, parameters):
        group = AgeGroup(0)
        # TODO: Here, we do not only index by age group, but also by the Infection State. Please add the missing parts:
        model.populations[] = parameters["InitiallyExposed"]
...
app._unparsable_cell(
    ...
    prior = pyabc.Distribution(
        ...
        # TODO: Set the following priors with plausible values
            TimeExposed = 
...
app._unparsable_cell(
    r"""
    # TODO: Create example results by running our simulation function on a random sample from the prior:
    example_results = 
    """,
    name="_"
)
...
app._unparsable_cell(
    r"""
    # TODO: Create a history by running the abc object until a minimum_epsilon of 0.1 is reached.
    history = 
    """,
    name="_"
)
...
@app.cell
def _():
    # TODO: Create plot_dead_data and plot_dead_mean functions
...
@app.cell
def _():
    # TODO: Create a run_simulation_with_params(df) function that gets a dataframerow df as input. It should extract the parameters and run a simulation with them for 60 days, before returning the results.
    return
